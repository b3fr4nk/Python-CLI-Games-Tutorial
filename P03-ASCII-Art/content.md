...
If you visit http://artii.herokuapp.com you can see that we can also add a `font` url parameter.
...
Then we can get a random one of these array with the `random.choice()` function.
...
Now we should be able to see our random font printed out!

Now we can make 3 requests using a for loop.
...
Its always harder to make software have a lot of options and configurability, so we're going to set ourselves a challenge of making things a little more complex.
...
First if there is no font specified we make the request with no font:
...
If we do it any other order, we'll get the wrong logic and we might see errors.
...
Our code currently repeats itself in on place in particular: calling the API. So let's pull that out into a function. Changing your code to be more DRY is called **refactoring**. So let's refactor our project a little bit by defining a function called `getAsciiArt`.
